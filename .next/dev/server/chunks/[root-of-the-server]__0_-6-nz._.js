...
async function GET(request) {
    ...
    const { data: slots, error } = await supabase.from('slots').select('*').order('start_time');
    if (error) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: error.message
        }, {
    ...
    if (date) {
        const { data: bookings } = await supabase.from('bookings').select('slot_id').eq('date', date).eq('status', 'active');
        const bookedCount = {};
        bookings?.forEach((b)=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0_-6-nz._.js.map